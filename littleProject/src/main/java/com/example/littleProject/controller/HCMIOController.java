...
public class HCMIOController {
    ...
    public List<HCMIO> getHCMIOByStock(@RequestParam String stock){
        List<HCMIO> hcmioList = this.hcmioService.getHCMIOByStock(stock);
        return hcmioList;
    }

    @GetMapping("/hcmio/SumOfNetAmt") //測試用
    public double SumOfNetAmt(@RequestParam String stock){
